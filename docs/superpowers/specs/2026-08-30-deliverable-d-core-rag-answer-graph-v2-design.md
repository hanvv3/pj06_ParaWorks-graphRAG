...
`affected_rows`는 unique `(row_kind,row_identity_hmac)` lexical order이고 실제 mutated rows와 exact해야
한다. lock/read-only invariant row는 넣지 않고 no-op UPDATE로 mutation을 꾸미지 않는다. cost child는 그
...
하고 affected set에는 없다. transition마다 append-only
...
| Kind | Authorization | Case | Dispatch/component | Safety/runtime binding | Exact mutated row kinds |
|---|---|---|---|---|---|
