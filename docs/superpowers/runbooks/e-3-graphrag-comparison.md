...
The pgvector control is E-1's recorded ordered serving IDs. E-2's actual
...
model-answer faithfulness, citation quality, latency, or paid cost.
...
`test_graph_retrieval_comparison.py` verifies all five cases, default-off
rollback, graph-unavailable/stale fallback, receipt identity/no extra provider
attempt, and (when `PARAWORKS_TEST_NEO4J_RESTART_PAUSE=1`) recreates the official
...
comparison/rollback tests passed in 6.10s. The controller-coordinated restart
